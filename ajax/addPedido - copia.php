<?php
	if (empty($_POST['comanda_random'])){
		...
		if($result_query_search_comanda){
			if(mysqli_num_rows($result_query_search_comanda) > 0){
				while($row = mysqli_fetch_assoc($result_query_search_comanda)){
					...
					if($result_query_search_pedidos){
						if(mysqli_num_rows($result_query_search_pedidos) > 0){
							...
							$query_insert_pedidos = "INSERT INTO t_pedidos(id_comanda, estado_comanda, estado, cod_cliente, num_mesa, articulo, rubro, cant_articulo, cant_nueva, importe, notas, fecha_abm, hora_abm)
							VALUES('$id_comanda', 'T', 'A', '$add_cliente', '$add_num_mesa', '$add_articulo', '$add_articulo_rubro', '$add_cantidad', '$add_cantidad', '$add_importe', '$add_nota', '$fecha_add', '$hora_add');";
				...
				updateComanda($conexion,$id_comanda);
	...
	function updateComanda($conexion,$id_comanda){
